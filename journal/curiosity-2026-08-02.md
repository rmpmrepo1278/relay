# Curiosity Log — 2026-08-02


## Curiosity — 00:15 UTC
Interests: self_hosting, ai_agents, llm_inference

🌱 **Adjacent to self hosting:** home assistant advanced
  • Missoula MT Real Estate & Homes For Sale - Zillow
    https://www.zillow.com/missoula-mt/?msockid=36f85b2f11bd6f0437fd4c8610c36e3b
🌱 **Adjacent to ai agents:** agent evaluation
  • 人人都在说的 Agent，到底是个什么东西？ - 知乎
    https://zhuanlan.zhihu.com/p/1999160608529072176
  • AI Agent Evaluation Frameworks 2026: How to Measure What Actually Matters
    https://skillgen.io/ai-agent-evaluation-frameworks-2026
🔀 **Intersection:** llm inference × self hosting
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐23819
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐13880
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • unicity-aos/aos-ce ⭐8575
    AOS Community Edition: the open agent operating system.
    https://github.com/unicity-aos/aos-ce
🎲 **Serendipity:**
  • Unraveling the mysteries of habit formation
    https://www.kyoto-u.ac.jp/en/research-news/2026-07-28
    (66 points on HN)

## Curiosity — 04:15 UTC
Interests: self_hosting, ai_agents, llm_inference

🔀 **Intersection:** ai agents × self hosting
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐23841
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐13891
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • unicity-aos/aos-ce ⭐8575
    AOS Community Edition: the open agent operating system.
    https://github.com/unicity-aos/aos-ce
🎲 **Serendipity:**
  • Running Kimi K3 on MI355X at Better Performance per Dollar Than B300
    https://www.wafer.ai/blog/kimi-k3-mi355x
    (136 points on HN)

## Curiosity — 08:15 UTC
Interests: self_hosting, ai_agents, llm_inference

🌱 **Adjacent to ai agents:** mcp protocol
  • 一文搞懂 MCP：从入门到实战（含本地项目 MCP Server 示例）
    https://blog.csdn.net/zgpeace/article/details/156002083
🔀 **Intersection:** self hosting × monitoring
  • Deploying Self-Hosted Sentry for Application Monitoring
    https://medium.com/@peterselvaponseelan/deploying-self-hosted-sentry-for-application-monitoring-15525e6f7e69
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐23866
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐13920
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • unicity-aos/aos-ce ⭐8575
    AOS Community Edition: the open agent operating system.
    https://github.com/unicity-aos/aos-ce
🎲 **Serendipity:**
  • Artificial Intelligence: Ars Notoria and the Promise of Instant Knowledge
    https://publicdomainreview.org/essay/ars-notoria/
    (71 points on HN)

## Curiosity — 08:30 UTC
Interests: self_hosting, ai_agents, llm_inference

🌱 **Adjacent to ai agents:** multi agent systems
  • ProShares UltraPro Short QQQ (SQQQ) - Yahoo Finance
    https://finance.yahoo.com/quote/SQQQ/
  • SQQQ | UltraPro Short QQQ - ProShares
    https://www.proshares.com/our-etfs/leveraged-and-inverse/sqqq
🔀 **Intersection:** monitoring × llm inference
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐23869
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐13920
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • unicity-aos/aos-ce ⭐8575
    AOS Community Edition: the open agent operating system.
    https://github.com/unicity-aos/aos-ce
🎲 **Serendipity:**
  • Go 1.27 Interactive Tour
    https://victoriametrics.com/blog/go-1-27/index.html
    (292 points on HN)

## Curiosity — 12:15 UTC
Interests: self_hosting, ai_agents, llm_inference

🔀 **Intersection:** self hosting × llm inference
  • HR Gateway - State of Michigan
    https://www.michigan.gov/mdcs/gateway
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐23888
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐13928
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • unicity-aos/aos-ce ⭐8575
    AOS Community Edition: the open agent operating system.
    https://github.com/unicity-aos/aos-ce

## Curiosity — 16:15 UTC
Interests: self_hosting, ai_agents, llm_inference

🌱 **Adjacent to ai agents:** multi agent systems
  • Multi-Agent AI Orchestration: Complete 2026 Guide
    https://www.claritywithai.org/2026/06/multi-agent-ai-orchestration-guide-2026.html
🔀 **Intersection:** self hosting × llm inference
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐23899
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐13937
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • unicity-aos/aos-ce ⭐8576
    AOS Community Edition: the open agent operating system.
    https://github.com/unicity-aos/aos-ce
🎲 **Serendipity:**
  • Show HN: NixOS-DGX-Spark – Nix and NixOS on the DGX Spark
    https://github.com/graham33/nixos-dgx-spark
    (81 points on HN)

## Curiosity — 20:15 UTC
Interests: self_hosting, ai_agents, llm_inference

🔀 **Intersection:** llm inference × monitoring
📄 **arXiv (ai agents):**
  • ExtractBench: A Benchmark for Schema-Guided Enterprise Document Extraction
    http://arxiv.org/abs/2607.29677v1
  • Development of FDD-ON: an Ontology for VAV HVAC System Fault Detection and Diagn
    http://arxiv.org/abs/2607.29657v1
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐23931
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐13949
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • unicity-aos/aos-ce ⭐8573
    AOS Community Edition: the open agent operating system.
    https://github.com/unicity-aos/aos-ce
🎲 **Serendipity:**
  • Californians' data deletion requests, DROP, become enforceable Aug. 1
    https://www.nbcsandiego.com/nbc-7-responds-2/californians-data-deletion-requests-drop-become-enforceable-aug-1/4054771/
    (104 points on HN)
