# Curiosity Log — 2026-07-21


## Curiosity — 00:16 UTC
Interests: self_hosting, ai_agents, llm_inference

🌱 **Adjacent to self hosting:** nixos
  • NixOS - Wikipedia
    https://en.wikipedia.org/wiki/NixOS
🌱 **Adjacent to ai agents:** mcp protocol
  • MCP 2026: The Complete Developer’s Guide to Model Context Protocol (With Code Ex
    https://essamamdani.com/blog/complete-guide-model-context-protocol-mcp-2026
  • The 2026 MCP Roadmap | Model Context Protocol Blog
    https://blog.modelcontextprotocol.io/posts/2026-mcp-roadmap/
🔀 **Intersection:** monitoring × ai agents
  • AI Observability Platform | Monitor Agent Behavior at Scale
    https://zenity.io/platform/ai-observability
📄 **arXiv (ai agents):**
  • Logical Judgments Under Pressure: Diagnosing Syllogistic Stability with Learned 
    http://arxiv.org/abs/2607.18228v1
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐21033
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐12684
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • deepseek-ai/DeepSpec ⭐6713
    DeepSpec: a full-stack codebase for training and evaluating speculative decoding
    https://github.com/deepseek-ai/DeepSpec
🎲 **Serendipity:**
  • Show HN: Immersive Gaussian Splat tour of grace cathedral, San Francisco
    https://vincentwoo.com/3d/grace_cathedral/
    (145 points on HN)

## Curiosity — 12:15 UTC
Interests: self_hosting, ai_agents, llm_inference

🌱 **Adjacent to self hosting:** nomad
  • Nomad GCS - Connected Mobile Operations Centers
    https://nomadgcs.com/
🌱 **Adjacent to ai agents:** multi agent systems
  • Get Multi
    https://getmulti.ai/
  • MULTI- Definition & Meaning - Merriam-Webster
    https://www.merriam-webster.com/dictionary/multi-
🔀 **Intersection:** llm inference × ai agents
📄 **arXiv (llm inference):**
  • The Many Senses of Visual Similarity: A Text-Prompted Image Perceptual Metric
    http://arxiv.org/abs/2607.18237v1
  • Patch Policy: Efficient Embodied Control via Dense Visual Representations
    http://arxiv.org/abs/2607.18236v1
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐21338
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐12756
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • deepseek-ai/DeepSpec ⭐6722
    DeepSpec: a full-stack codebase for training and evaluating speculative decoding
    https://github.com/deepseek-ai/DeepSpec

## Curiosity — 16:16 UTC
Interests: self_hosting, ai_agents, llm_inference

🌱 **Adjacent to self hosting:** nixos
  • NixOS Wiki/getting started - Official NixOS Wiki
    https://wiki.nixos.org/wiki/NixOS_Wiki/getting_started
  • NixOS Wiki - Official NixOS Wiki
    https://wiki.nixos.org/wiki/NixOS_Wiki/en
🌱 **Adjacent to ai agents:** multi agent systems
  • How to Build Multi-Agent Systems: Complete 2026 Guide - DEV Community
    https://dev.to/eira-wexford/how-to-build-multi-agent-systems-complete-2026-guide-1io6
  • Multi-Agent Systems (MAS) in 2026: The Complete Guide to Architecture, Models, U
    https://santageai.com/learn/concepts/multi-agent-systems
🔀 **Intersection:** monitoring × ai agents
📄 **arXiv (llm inference):**
  • Causal Discovery on Irregular Time Series
    http://arxiv.org/abs/2607.18226v1
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐21365
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐12772
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • deepseek-ai/DeepSpec ⭐6725
    DeepSpec: a full-stack codebase for training and evaluating speculative decoding
    https://github.com/deepseek-ai/DeepSpec
🎲 **Serendipity:**
  • Apple Private Cloud Compute SoC 3 audit reports
    https://support.apple.com/guide/certifications/apple-private-cloud-compute-soc-3-audit-apc95a31b9d8/web
    (83 points on HN)

## Curiosity — 20:15 UTC
Interests: self_hosting, ai_agents, llm_inference

🌱 **Adjacent to ai agents:** mcp protocol
  • dashboard.mux.com
    https://dashboard.mux.com/login
🔀 **Intersection:** self hosting × ai agents
  • 8 Best Self-Hosted AI Agent Platforms for 2025 | Fastio
    https://fast.io/resources/best-self-hosted-ai-agent-platforms/
📄 **arXiv (ai agents):**
  • Copy Less, Ground More: Overcoming Repetitive Copying in Long-Context Reasoning 
    http://arxiv.org/abs/2607.19345v1
  • Appearance Pointers -- Multimodal Region Control of Diffusion Transformers
    http://arxiv.org/abs/2607.19344v1
⭐ **Trending on GitHub:**
  • xai-org/grok-build ⭐21462
    SpaceXAI's coding agent harness and TUI. Fullscreen, mouse interactive, extensib
    https://github.com/xai-org/grok-build
  • langchain-ai/openwiki ⭐12795
    OpenWiki is a CLI that writes and maintains agent documentation for your codebas
    https://github.com/langchain-ai/openwiki
  • deepseek-ai/DeepSpec ⭐6729
    DeepSpec: a full-stack codebase for training and evaluating speculative decoding
    https://github.com/deepseek-ai/DeepSpec
🎲 **Serendipity:**
  • Jack Dorsey launches Buzz to combine team chat, AI agents and Git hosting
    https://runtimewire.com/article/jack-dorsey-block-buzz-team-chat-ai-agents-git
    (259 points on HN)
