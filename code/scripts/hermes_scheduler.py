...
#             Schedule(minute="*/5"), description="Gateway health recovery", tags=["monitor"]), # legacy-disabled by Relay 2026-09-03
        ...
        Job("core_patch", p(f"{h}/scripts/core_patch.py"),
            Schedule(minute="30", hour="4", day_of_week="5"), timeout=3600,
        ...
        Job("weekly_radiator", p(f"{h}/scripts/life_radiator.py"),
            Schedule(minute="15", hour="13", day_of_week="6"), timeout=120,
        ...
        Job("health_checkin", f"python3 {h}/scripts/health_tracker.py nudge",
            Schedule(minute="0", hour="22", day_of_week="6"), timeout=30, description="Weekly health check-in prompt (Sun)", tags=["personal", "health"]),
        Job("social_presence", f"python3 {h}/scripts/social_presence.py digest",
            Schedule(minute="30", hour="11", day_of_week="6"), timeout=60, description="Weekly social presence digest (GitHub activity)", tags=["social"]),
        ...
        Job("homelab_optimizer", p(f"{h}/scripts/homelab_optimizer.py"),
            Schedule(minute="0", hour="10", day_of_week="6"), timeout=120, description="Tune resource limits + retention", tags=["homelab"]),
        ...
        Job("career_coach_weekly", p(f"{h}/scripts/career_coach.py weekly-send"),
            Schedule(minute="30", hour="13", day_of_week="6"), timeout=60, description="Career coach weekly review (Telegram)", tags=["career"]),
...
Job("homelab_reporter", p(f"{h}/scripts/homelab_reporter.py"),
        ...
        Job("gdrive_keepalive", s(f"{h}/scripts/gdrive_keepalive.sh"),
            Schedule(minute="45", hour="*/6"), timeout=400, description="GDrive token refresh", tags=["backup"]),
        ...
        Job("docker_ghost_check", s(f"{h}/scripts/docker_ghost_check.sh"),
            Schedule(minute="*/30"), timeout=120, description="Detect ghost/orphaned containers", tags=["monitor"],
            healthchecks_uuid=HC_UUID_DOCKER_GHOST_CHECK),
        Job("cve_scan", s(f"{h}/scripts/cve_scan.sh"),
            Schedule(minute="30", hour="4", day_of_week="6"), timeout=3600, description="Weekly trivy scan of key images", tags=["security"],
        ...
        Job("weekly_review", p(f"{h}/hermes-agent/scripts/weekly_review.py"),
            Schedule(minute="0", hour="17", day_of_week="4"), timeout=180, description="Weekly review (DISABLED 2026-09-12: script missing)", tags=["weekly"], enabled=False),
                # ── Maintenance ──
        Job("docker_build_prune", s(f"{h}/scripts/docker_build_prune.sh"),
            Schedule(minute="30", hour="3", day_of_week="6"), timeout=300, description="Prune Docker build cache weekly", tags=["maintenance"]),
...
Job("weekly_audit", p(f"{h}/scripts/weekly_audit.py"),
            Schedule(minute="0", hour="13", day_of_week="6"), timeout=180, description="Weekly audit", tags=["weekly"]),
        Job("weekly_health", p(f"{h}/scripts/weekly_health_digest.py"),
            Schedule(minute="0", hour="8", day_of_week="6"), timeout=300, description="Weekly health digest to Telegram", tags=["maintenance"], circuit="telegram_bridge"),
        Job("volume_restore_test", s(f"{h}/scripts/volume_restore_test.sh"),
            Schedule(minute="30", hour="5", day_of_week="6"), timeout=600, description="Prove volume backups restorable", tags=["maintenance"]),
        Job("dr_runbook_check", s(f"{h}/scripts/dr_runbook_check.sh"),
            Schedule(minute="0", hour="6", day_of_week="6"), timeout=300, description="Verify DR runbook preconditions", tags=["maintenance"]),
        Job("upgrade_claude", "npm install -g --allow-scripts=@anthropic-ai/claude-code @anthropic-ai/claude-code@latest",
            Schedule(minute="0", hour="14", day_of_week="5"), timeout=120, description="Claude upgrade (user-level npm-global; sudo secure_path lacks npm)", tags=["maintenance"]),
        Job("cert_renew", f"docker run --rm -v /home/rohit/services/traefik/certs:/certs -e DUCKDNS_TOKEN=$(cat /home/rohit/.duckdns_token 2>/dev/null) goacme/lego:v3.7.0 --path /certs --email rohitmishra1278@gmail.com --dns duckdns --domains '*.chagulihome.duckdns.org' renew --days 30",
            Schedule(minute="0", hour="4", day_of_week="6"), timeout=300, description="SSL cert renewal", tags=["security"], shell=True),

        # ── SQLite maintenance (Sunday 3am) ──
        Job("sqlite_vacuum", f"sqlite3 {h}/state.db VACUUM; sqlite3 {h}/data/unified_memory.db VACUUM",
            Schedule(minute="0", hour="3", day_of_week="6"), timeout=300, description="SQLite vacuum", tags=["maintenance"], shell=True),

        # ── Self-evolution (Sunday 4am) ──
        Job("self_evolution", p(f"{h}/scripts/self_evolution.py"),
            Schedule(minute="0", hour="4", day_of_week="6"), timeout=300, description="Self-evolving fix strategies", tags=["maintenance"]),
        Job("ace_curate", p(f"{h}/scripts/ace_playbook.py curate"),
            Schedule(minute="30", hour="4", day_of_week="6"), timeout=120, description="Curate ACE playbook candidates", tags=["maintenance"]),
        ...
        Job("auto_followup", p(f"/home/rohit/projects/career-ops/scripts/auto_followup.py --days 14 --send"),
            Schedule(minute="30", hour="10", day_of_week="0"), timeout=120, description="Auto followup", tags=["career"]),
        Job("company_scan", p(f"/home/rohit/projects/career-ops/scripts/scan_companies.py 5 --json"),
            Schedule(minute="0", hour="14", day_of_week="0-4"), timeout=180, description="Company scan (afternoon, internet guaranteed)", tags=["career"]),
...
# DISABLED         # Note: telegram_bot runs in screen session, not scheduled (daemon)
        ...
        Job("morning_brief_pa", p(f"{h}/scripts/personal_brief.py morning"),
        ...
        Job("evening_brief_pa", p(f"{h}/scripts/personal_brief.py evening"),
        ...
        Job("preempt_triggers", f"python3 /home/rohit/.hermes/scripts/preempt_triggers.py --run",
            Schedule(minute="*/20"), timeout=60, description="PA Tier-2 preemption triggers (trip checklists, due/bill alerts, optional weather)", tags=["pa", "proactive"], circuit="telegram_bridge"),
        Job("action_tuning", f"python3 /home/rohit/.hermes/scripts/action_loop.py tune",
            Schedule(minute="30", hour="8", day_of_week="6"), timeout=60, description="PA Tier-3 weekly action-loop learning ledger tuning brief", tags=["pa", "learning"], circuit="telegram_bridge"),
        ...
        Job("debloat", s(f"{h}/scripts/debloat.sh"),
            Schedule(minute="0", hour="4", day_of_week="6"), timeout=300, description="Weekly debloat sweep", tags=["maintenance"]),
        ...
        Job("doc_drift_check", f"python3 {h}/scripts/claude_md_sync.py && python3 {h}/scripts/doc_drift_check.py",
        ...
        Job("research_indexer", p(f"{h}/scripts/research_indexer.py"),
            Schedule(minute="0", hour="4", day_of_week="6"), timeout=120,
